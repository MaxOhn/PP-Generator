...
public class scoreEmbed {
    ...
    private static boolean prepareFiles(int mapID, int mapSetID) {
        boolean success = true;
        if (!new File(secrets.thumbPath + mapSetID + "l.jpg").isFile())
            success = Main.fileInteractor.downloadMapThumb(mapSetID);
    ...
    private static String updateBuilderTitle(Beatmap m, Set<Mod> mods, String starRating) {
    ...
    public static void embedScoreCompare(MessageReceivedEvent event, User user, Beatmap map, BeatmapScore score,
        ...
        boolean fileSuccess = prepareFiles(map.getBeatmapId(), map.getBeatmapSetId());
            ...
            try {
                ...
                eb.clearFields().setTimestamp(null)
                        .addField(new MessageEmbed.Field(rank + mods + "\t" +
                                ...
                                performance.getAcc() + "%)\t" + howLongAgo(score.getDate()), "**" + df.format(score.getPp()) +
                                "pp**/" + performance.getPpMax() + "PP\t[ " + score.getMaxCombo() + "x/" +
                ...
                event.getTextChannel().sendFile(bgThumb,
                    ...
                    try {
                        ...
                        eb.clearFields().setTimestamp(null)
                                .addField(new MessageEmbed.Field(rank + mods + "\t" +
                                        NumberFormat.getNumberInstance(Locale.US).format(score.getScore()) + "\t(" +
                                        performance.getAcc() + "%)\t" + howLongAgo(score.getDate()), "**"
                                        + (score.getRank().equals("F") ? "-" : (df.format(score.getPp()) + "pp")) + "**/" +
                                        performance.getPpMax() + "PP\t[ " + score.getMaxCombo() + "x/" +
        ...
        event.getTextChannel().sendFile(bgThumb,"thumb.jpg", mb.build()).queue(message -> {
            try {
                ...
                eb.clearFields().setTimestamp(null)
                        .addField(new MessageEmbed.Field(rank + mods + "\t" +
                                NumberFormat.getNumberInstance(Locale.US).format(score.getScore()) + "\t(" +
                                performance.getAcc() + "%)\t" + howLongAgo(score.getDate()), "**" + performance.getPp() +
                                "pp**/" + performance.getPpMax() + "PP\t[ " + score.getMaxCombo() + "x/" +
                                map.getMaxCombo() + "x ]\t { " + score.getCount300() + " / " + score.getCount100() + " / " +
                                score.getCount50() + " / " + score.getCountMiss() + " }", false));
                eb.setTitle(updateBuilderTitle(map, null, performance.getStarRating()), "https://osu.ppy.sh/b/" + map.getBeatmapId());
        ...
        event.getTextChannel().sendFile(bgThumb,"thumb.jpg", mb.build()).queue(message -> {
            try {
                ...
                eb.clearFields().setTimestamp(null)
                        .addField(new MessageEmbed.Field(rank + mods + "\t" +
                                NumberFormat.getNumberInstance(Locale.US).format(score.getScore()) + "\t(" +
                                performance.getAcc() + "%)\t" + howLongAgo(score.getDate()), "**" + (score.getRank().equals("F") ?
    ...
    public static void embedScores(MessageReceivedEvent event, User user, Beatmap map, Collection<BeatmapScore> scores) {
        ...
        for (BeatmapScore score : scores) {
            ...
            name = "**" + idx++ + ".** " + rank + mods + "\t[" + performance.getStarRating() + "★]\t" +
                    NumberFormat.getNumberInstance(Locale.US).format(score.getScore()) + "\t(" + performance.getAcc() + "%)";
            switch (map.getMode()) {
                case STANDARD:
    ...
    public static void embedTopScores(MessageReceivedEvent event, User user, Collection<UserScore> scores,
                                      ArrayList<Beatmap> maps, GameMode mode) {
        ...
        for (UserScore score : scores) {
            map = maps.get(idx - 1);
            prepareFiles(map.getBeatmapId(), map.getBeatmapSetId());
            if (!description.toString().equals("")) description.append("\n");
            performance = new Performance().map(map).userscore(score).mode(mode);
            rank = event.getJDA().getGuildById(secrets.devGuildID)
                    .getEmoteById(utilOsu.getRankEmote(score.getRank()).getValue()).getAsMention();
            mods = modString(score.getEnabledMods());
            description.append("**").append(idx++).append(".** [**")
                    .append(map.getTitle()).append("[").append(map.getVersion()).append("]**](https://osu.ppy.sh/b/")
                    .append(map.getBeatmapId()).append(")").append(mods.equals("") ? "" : "**" + mods + "**").append(" [")
                    .append(performance.getStarRating()).append("★]\n ")
                    .append(rank).append(" **").append(performance.getPp()).append("pp**/").append(performance.getPpMax())
                    .append("PP ~ (").append(performance.getAcc()).append("%) ~ ")
                    .append(NumberFormat.getNumberInstance(Locale.US).format(score.getScore())).append("\n  [ ")
                    .append(performance.getCombo()).append("x/").append(performance.getMaxCombo()).append("x ] ~ { ");
            switch (mode) {
                case STANDARD:
                    description.append(score.getCount300()).append(" / ").append(score.getCount100()).append(" / ")
                            .append(score.getCount50());
                    break;
                case OSU_MANIA:
                    description.append(score.getCountGeki()).append(" / ").append(score.getCount300()).append(" / ")
                            .append(score.getCountKatu()).append(" / ").append(score.getCount100()).append(" / ")
                            .append(score.getCount50());
                    break;
                case TAIKO:
                    description.append(score.getCount300()).append(" / ").append(score.getCount100());
                    break;
                default: break;
            }
            description.append(" / ").append(score.getCountMiss()).append(" } ~ ").append(howLongAgo(score.getDate()));
        }
